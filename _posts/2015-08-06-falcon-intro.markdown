...
## 数据采集
在介绍"服务模块"时，我可以忽略了`Falcon-Agent`这一组件。`Falcon-Agent`是监控系统提供的、用于采集监控数据的组件，顺便也做一点Proxy的工作。那么，监控系统是否有必要亲力亲为地负责监控数据采集？
...
有一些指标很通用，在运维工作实践中进场被用到。这类指标大多与机器相关，主要包括Cpu、Load、Mem、Disk、IO、Net、Core、Port、Process、NTP、DNS等。为了方便用户使用，监控系统要帮忙采集这些数据: 做一个本机数据采集的代理Agent、安装到机器上，然后由Agent主动采集本机信息、自动push给监控系统。

>2.agent插件机制

Agent支持的采集项，基本能覆盖到常见的监控需求。但仍然会存在一些特殊的业务需求，或者不适合内置于Agent的采集项，为了cover这些需求，插件机制应运而生。插件是符合一定规范的，由用户开发的数据采集脚本或者可执行程序，Agent会负责周期性的调度这些插件，并将插件运行的输出，推送到监控系统；用户可以通过配置接口，指定哪些机器需要执行哪些插件。插件机制，是对Agent数据采集能力的扩展，更加灵活多样。

需要指出的是，生效插件需要采集链路上的多个组件配置完成，这种高复杂度的机制 往往会成为很多bug的丰富来源。

>3.自维护采集服务

不能或者不方便 通过Agent采集的监控指标，需要用户自己完成数据采集、自己将数据push到监控系统。为此，监控系统需要制定灵活的、确定的数据模型，来支持用户自定义数据的采集；同时，还要提供简单易用的数据接收器、方便用户push数据。

在庞大的用户自维护采集服务中，有一些是较通用的需求，比如网络设备监控、Java应用监控、Nginx监控、Mysql监控、HBase监控、Redis监控等。具体的业务部门，需要负责上述指标的采集工作；监控团队可以协助搞定，但不应该接手上述工作。

>4.Nodata监控

对于用户自维护的采集服务，监控服务的存活是一个通用需求。为此，监控系统设计了Nodata功能，用户监控数据采集服务(包括Agent)的可用性。
