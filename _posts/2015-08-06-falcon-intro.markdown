...
Graph组件从Transfer接收分片数据，然后将数据写入本地的Rrdtool。在数据每次存入的时候，Rrdtool会自动进行采样、归档，策略如下。历史数据保存5年。同时为了不丢失信息量，数据归档的时候，会按照平均值采样、最大值采样、最小值采样存三份。以一分钟的上报周期为例子，下面是我们采用的归档策略:
```
	// 1分钟一个点存 12小时
...
<br />
## 报警配置
监控报警的三要素，为: 监控指标的数据，判断条件，报警。判断条件+报警=报警策略。
...
<br />
## 结束语
总的来说，Falcon在设计理念上是非常先进的，领先传统厂商至少一代。主要体现在以下几点:
