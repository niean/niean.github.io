...
```sql
# rid=`role`.getIdOfRole('cop.xiaomi_owt.inf', 'dev.common')
...
通常，我们在业务树的根节点`cop.xiaomi`，授予，各角色以最大权限；同时，我们在业务树子节点，以覆盖的方式，重新定义一些角色的权限。这样，既可以 让用户方便地 在各业务树子节点**获取权限**，又可以 让用户方便地在各业务树节点**定制权限**。
