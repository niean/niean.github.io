...
- 服务化原则：功能拆分、实现微服务或小服务化，接口通信、做到高内聚高复用，服务抽象、实现基于服务流量的控制和治理
    - 模块拆分，按功能模块拆分业务代码、实现微服务或小服务架构，提升迭代效率(不同生命周期彼此独立迭代)
    - 接口通信，以接口契约定义业务关系(如IDL)、以标准协议实现彼此互通(如HTTP)，做到模块功能的高内聚、高复用
    - 服务抽象，抽象业务逻辑(如DDD)，做到基于服务流量的控制和治理，如切限降熔、灰度、反压、零信任等
    - 备注，小服务是介于单体和微服务之间的形态，共享少量数据，一般是为了提升性能、降低调用复杂度
- 弹性原则：容量可以随业务量大小自动调整，无需囤积固定的软硬件资源
    - 水平扩展，业务架构支持多实例部署、支持多单元分区，并且做到流量调度、均衡
    - 自动部署，根据预设的规则、检测条件自动伸缩
    - 主要受益，从容应对突增流量、降低IT成本
- 可观测原则：通过日志、指标、追踪、事件，提供统一监控视图，加强数据关联分析，有效降低故障的MTTR
...
- 自动化原则：通过配置数据自描述、面向终态的交付，让自动化工具理解交付目标和环境差异，实现整个软件交付和运维的自动化
    - 产生原因，微服务、Devops、容器、第三方组件大量应用，软件的组件规模变大、技术栈更多样，使得软件交付变的更复杂，不自动化只能死路一条
    - 设计原则，标准化、面向终态、关注点分离(不同工种关注不同的事项)、面向失败等
    - 注意事项，自动化系统的执行过程，同样需要遵循人工变更的最佳实践，做到灰度执行、结果可观测、变更可回滚、影响可追溯
- 可持续原则：软件世界是不断变化的、架构演进本身是一个不断尝试的过程，需要继承优秀的既有理念、做叠加式创新
