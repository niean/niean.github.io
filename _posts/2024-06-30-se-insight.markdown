...
工程系统，遵循`输入-转化-输出`模型。输入物质、能源，经由信息系统的过程转换，输出人类所需的特定功效。
...
## 管理
#### 2026.01.30 横向格局
横向思维催发的格局，是淳朴的、原始的、低阶的格局。

横向格局，本质不是看清终态后的利他主义，而是无知状态下的共情泛滥、是资源富态下的边界缺失。
...
- 组织设计：高效的组织建设，靠的是正向的职能设计、人才筛选，而不是以下级为中心的温情讨好、兜底提拉。PUA管理者高材低就、接受环境错配，且美其名曰培养的，始作俑者其无后乎
...
横向解构，路径是：拆解横向事务 -> 提取通用要素 -> 按纵向目标重组 -> 沉淀纵向能力。
